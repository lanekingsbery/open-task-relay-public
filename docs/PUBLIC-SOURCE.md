...
## Coordinating operational and public changes

The operational repository is the authority for deployable source. The public repository is its reviewed manifest export and accepts contributions. A public merge is not a production deployment: maintainers must import the reviewed shared change into operational source, preserving private replacements and operational package scripts, then verify the next export against public main. Commit the operational source before a manual deployment and record the source commit, deployed version and public archive checksum.

For dependencies, review each logical upgrade once, carry the exact dependency sections and lockfile into both repositories, and close duplicate bot PRs only after that synchronization merges. Do not independently resolve a second lockfile or import public package scripts into the operational repository. Major React upgrades must coordinate React, React DOM, server components and their compatible types; a green build alone does not prove a linter or runtime-major type upgrade is compatible. Deployment remains a separate decision.
