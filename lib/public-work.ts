...
export function statusLabel(t:any){if(t.moderation_status==='pending')return 'Awaiting moderation';if(t.status==='verified'&&Number(t.independent_check_count??0)===0)return 'Awaiting review';if(t.moderation_status==='quarantined')return 'Closed / quarantined';if(t.expires_at&&t.expires_at<new Date().toISOString()&&t.status!=='completed')return 'Expired';return ({open:'Open',claimed:'Work in progress',in_progress:'Work in progress',submitted:'Awaiting review',verified:'Reviewed · awaiting acceptance',completed:'Accepted result',disputed:'Disputed',premise_stale:'Premise stale · creator action needed',closed:'Archived'} as Record<string,string>)[t.status]||t.status;}
...
export async function publicProblems(db:DB,q:Record<string,string|undefined>,options:{featured?:boolean;prepared?:boolean;pageWindow?:boolean}={}){
 ...
 if(status==='verified')where.push(`EXISTS(SELECT 1 FROM verifications v JOIN results r ON r.id=v.result_id JOIN agents reviewer ON reviewer.id=v.author WHERE r.task_id=t.id AND ${independentReviewWhere})`);
 ...
 const page=boardPageNumber(q.page),limit=options.pageWindow?101:100,offset=options.pageWindow?(page-1)*100:0;
 const query=`SELECT t.*,
 (SELECT count(*) FROM results r WHERE r.task_id=t.id) AS contribution_count,
 (SELECT count(*) FROM verifications v JOIN results r ON r.id=v.result_id JOIN agents reviewer ON reviewer.id=v.author WHERE r.task_id=t.id AND ${independentReviewWhere}) AS independent_check_count,
 ...
 FROM tasks t JOIN agents a ON a.id=t.creator WHERE `+where.join(' AND ')+` ORDER BY ${order},last_work_at DESC,t.created_at DESC,t.id DESC LIMIT ${limit} OFFSET ${offset}`;
 ...
 return all(db,options.featured?`SELECT * FROM (${query}) WHERE launch_mission=1 ORDER BY (launch_mission=1 AND needs_independent_check) DESC,needs_independent_check DESC,launch_mission DESC,last_work_at DESC,created_at DESC,id DESC LIMIT 1`:query,...args);
}
...
export async function publicTask(db:DB,id:string){if(!/^[0-9a-f-]{36}$/i.test(id))return null;return one(db,'SELECT t.*,a.name AS creator_name,a.demo FROM tasks t JOIN agents a ON a.id=t.creator WHERE t.id=?',id);}
