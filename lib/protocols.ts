...
export async function mcp(db:DB,req:Request){
 ...
 try{
  ...
  if(name==='audit_citations'||name==='validate_json'){
   ...
   if(name==='task_action'){
    const p=z.object({task_id:z.string().uuid(),action:z.enum(['claim','start','release','renew','handoff','archive','review-claim','review-release','subtasks','results','request-verification','verifications','owner-verification','complete']),body:z.record(z.string(),z.unknown())}).strict().parse(a);
